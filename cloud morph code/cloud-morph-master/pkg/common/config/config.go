...
type Config struct {
	...
	// EnableLegacyDemoApp keeps the original cloud-morph behavior of launching
	// a demo app (path/appFile) and blocking server startup on its first RTP
	// packet. Defaults to false: the Xenia bridge (xenia_bridge.go) registers
	// its own on-demand, per-session /streams/* routes instead, which never
	// blocks HTTP startup.
	EnableLegacyDemoApp bool `yaml:"enableLegacyDemoApp"`
}
