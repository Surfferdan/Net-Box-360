...
func NewServer() *Server {
	...
	if cfg.EnableLegacyDemoApp {
		// Opt-in legacy behavior: launches a demo app and blocks startup on its
		// first RTP packet. Off by default so the API always binds immediately.
		log.Println("Spawn cloudapp server (legacy demo app, enableLegacyDemoApp=true)")
		cappServer := cloudapp.NewServerWithHTTPServerMux(cfg, r, svmux)
		server.cappServer = cappServer
		cappServer.Handle()
	} else {
		// Default path: register the on-demand Xenia bridge. No app is launched
		// and no capture starts until a session POSTs /streams/start.
		log.Println("Registering Xenia bridge routes (/healthz, /streams/*)")
		server.xeniaBridge = NewXeniaBridge(cfg)
		server.xeniaBridge.RegisterRoutes(r)
	}
