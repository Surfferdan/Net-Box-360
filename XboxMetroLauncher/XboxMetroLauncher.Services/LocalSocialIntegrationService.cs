...
public sealed class LocalSocialIntegrationService : ISocialIntegrationService
{
	...
	public static FriendProfile MapToLocalFriend(SocialFriend friend)
	{
		return new FriendProfile
		{
			...
			Country = "Offline",
			DashX360Id = string.Empty,
			DashX360FriendCode = string.Empty,
			DiscordUserId = string.Empty
		};
	}

	private static string ExtractDiscordUserId(SocialFriend friend)
	{
		if (friend.Id.StartsWith("discord:", StringComparison.OrdinalIgnoreCase))
		{
			return friend.Id.Substring("discord:".Length).Trim();
		}
		const string text = "Discord ";
		if (!string.IsNullOrWhiteSpace(friend.IdentityDetailText) && friend.IdentityDetailText.StartsWith(text, StringComparison.OrdinalIgnoreCase))
		{
			return friend.IdentityDetailText.Substring(text.Length).Trim();
		}
		return string.Empty;
	}

	private static string ExtractDeviceId(SocialFriend friend)
	{
		if (friend.Source == SocialFriendSource.DashX360 && friend.Id.StartsWith("dashx360:", StringComparison.OrdinalIgnoreCase))
		{
	...
	private static string ExtractFriendCode(string identityDetailText)
	{
		const string text = "DashX360 ";
		if (!string.IsNullOrWhiteSpace(identityDetailText) && identityDetailText.StartsWith(text, StringComparison.OrdinalIgnoreCase))
		{
			return identityDetailText.Substring(text.Length).Trim();
		}
		return string.Empty;
	}
